...
Eigen::Matrix<double, 15, 1> IntegratedLocalizer::state_diff(
    const State& state_a,
    const State& state_b) {
  Eigen::Matrix<double, 15, 1> diff;
  diff.segment(0, 3) = state_a.pos - state_b.pos;
  diff.segment(3, 3) = state_a.vel - state_b.vel;
  diff.segment(6, 3) = orientation_diff(state_a.ori, state_b.ori);
  diff.segment(9, 3) = state_a.a_bias - state_b.a_bias;
  diff.segment(12, 3) = state_a.w_bias - state_b.w_bias;
...
// --------------------------------------------------------------------------------------------------------
// Computes state predction based on propagated sigma points
void IntegratedLocalizer::compute_pred_state() {
  ...
  if(delta_t < 5){
    ...
    State state_use = state_history_update.back();
    ...
    Eigen::Matrix<double, 16, 16> p_state_cov_ = state_history_updated_cov_.back();
...
void IntegratedLocalizer::state_update_using_pred_sigma_points_lidar() {
  ...
  // 1, the sigma points are projected through the observation function h
  std::vector<Pose> pred_measurements;
  for (auto sp : pred_sigma_points_) {
    pred_measurements.push_back(lidar_measurement_H_function(sp));
  }

  // 2, the weighted sigma points are recombined to produce the predicted measurement and predicted
  // measurement covariance

  // 1)
  // Predicted lidar measurement is the first sigma point because the other
  // sigma points are symmetrically located around it
  pred_lidar_measurement_ = pred_measurements[0];

  // 2)

  // Computes lidar measurement covariance and cross covariance between lidar
  // measurement and state
  pred_lidar_measurement_cov_ = Eigen::Matrix<double, 6, 6>::Zero();
  pred_state_lidar_cross_cov_ = Eigen::Matrix<double, 15, 6>::Zero();
  for (int i = 0; i < cov_weights_.size(); ++i) {
    Pose pose = pred_measurements[i];
    State sp = pred_sigma_points_[i];
    double cov_w = cov_weights_[i];

    Eigen::Matrix<double, 15, 1> pred_state_diff = state_diff(sp, pred_state_);
    Eigen::Matrix<double, 6, 1> pred_measure_diff;
    pred_measure_diff = measurement_diff(pose, pred_lidar_measurement_);

    pred_lidar_measurement_cov_ += pred_measure_diff
                                   * pred_measure_diff.transpose() * cov_w;
    pred_state_lidar_cross_cov_ += pred_state_diff
                                   * pred_measure_diff.transpose() * cov_w;
  }
  pred_lidar_measurement_cov_ += lidar_noise_;

  // Computes kalman gain
  Eigen::Matrix<double, 15, 6> Kalman_gain = pred_state_lidar_cross_cov_
                                        * pred_lidar_measurement_cov_.inverse();

  // Computes difference between real and predicted measurements
  Eigen::Matrix<double, 6, 1> real_measure_diff;
  real_measure_diff = measurement_diff(lidar_measurement_,
                                       pred_lidar_measurement_);


  // Computes state update vector
  Eigen::Matrix<double,15,1> update_vector = Kalman_gain * real_measure_diff;

  // Updates state and state covariance
  updated_state_ = increment_state(pred_state_, update_vector);


  updated_state_cov_ = pred_state_cov_
          - Kalman_gain * pred_lidar_measurement_cov_ * Kalman_gain.transpose();
  //std::cout << "updated state covariance = " << updated_state_cov_ << std::endl;
  ...
  updated_state_.timestamp = pred_state_.timestamp;
  //ROS_INFO("Updated state using lidar pose measurement!");
}
*/
void IntegratedLocalizer::state_update_using_pred_sigma_points_lidar() {
  ...
  Eigen::Matrix<double, 7, 7> state_cov_pred = state_history_pred_cov.back();
...
IntegratedLocalizer::Pos IntegratedLocalizer::state_to_pose(
...
/*
// --------------------------------------------------------------------------------------------------------
// Converts imu pose to camera pose
IntegratedLocalizer::Pose IntegratedLocalizer::imu_pose_to_camera_pose(
    Pose imu_pose) {
  Pose camera_pose;
  camera_pose.ori = imu_pose.ori * imu_to_camera_.ori;
  camera_pose.pos = imu_pose.ori * imu_to_camera_.pos + imu_pose.pos;
  camera_pose.timestamp = imu_pose.timestamp;
...
void IntegratedLocalizer::gps_pose_callback(
    const geometry_msgs::PoseStamped::ConstPtr& gps_pose_msg) {
      ...
      lidar_points_manipulation(velocity_deltax, velocity_deltay, delta_time);
...
void IntegratedLocalizer::lidar_points_manipulation(double vx, double vy, double dt){
  if(is_gnss_initialized != 1){
    State locate;
    locate.pos(0) = gps_measurement.pos(0) + vx*dt;
    locate.pos(1) = gps_measurement.pos(1) + vy*dt;
    locate.pos(2) = gps_measurement.pos(2);
    locate.ori = gps_measurement.ori;
    locate.vel(0) = vx;
    locate.vel(1) = vy;
    locate.vel(2) = 0;
    locate.a_bias = Eigen::Vector3d::Zero();
    locate.w_bias = Eigen::Vector3d::Zero();
    locate.timestamp = gps_measurement.timestamp;
...
void IntegratedLocalizer::lidar_points_manipulation2(){
  State locate;
  locate.pos  lidar_measurement.pos;
  locate.ori = lidar_measurement.ori;
  locate.vel = Eigen::Vector3d::Zero();
  locate.a_bias = Eigen::Vector3d::Zero();
  locate.w_bias = Eigen::Vector3d::Zero();
  locate.timestamp = lidar_measurement.timestamp;
  state_history_.push_back(locate);
  check_state_history_size();
}
// --------------------------------------------------------------------------------------------------------
...
IntegratedLocalizer::Pose IntegratedLocalizer::get_latest_lidar_pose() {
  State latest_state = state_history_final_.back();
  Pose imu_pose;
...
// --------------------------------------------------------------------------------------------------------
// Callback function for camera image ros topic subscription to be implemented
// but may not be necessary
void IntegratedLocalizer::camera_data_callback(
    const sensor_msgs::Image::ConstPtr& image) {
  int sec = image->header.stamp.sec;
  int nsec = image->header.stamp.nsec;
  double image_timestamp = static_cast<double>(sec)
                           + static_cast<double>(nsec) * 1e-9;

  ROS_INFO("Received image at %f!", image_timestamp);
...
// --------------------------------------------------------------------------------------------------------
// Callback function for lidar data ros topic subscription to be implemented
// but may not be necessary
void IntegratedLocalizer::lidar_data_callback(
    const sensor_msgs::PointCloud2::ConstPtr& lidar) {
  int sec = lidar->header.stamp.sec;
  int nsec = lidar->header.stamp.nsec;
  double lidar_timestamp = static_cast<double>(sec)
                           + static_cast<double>(nsec) * 1e-9;

  //ROS_INFO("Received lidar data at %f!", lidar_timestamp);

  if (is_state_initialized_) {
  }
}

// Converts camera pose to imu pose
IntegratedLocalizer::Pose IntegratedLocalizer::camera_pose_to_imu_pose(
    Pose camera_pose) {
  Pose imu_pose;
  imu_pose.ori = camera_pose.ori * camera_to_imu_.ori;
  imu_pose.pos = camera_pose.ori * camera_to_imu_.pos + camera_pose.pos;
  imu_pose.timestamp = camera_pose.timestamp;
