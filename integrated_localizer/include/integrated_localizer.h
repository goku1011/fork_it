  ...
  public:
    ...
    // Rotation matrix from GPS World coordinate to GPS local coordinate
  	Eigen::Matrix3d Q_GW_G_0_;
		...
		std::deque<Eigen::Matrix<double, 16, 16>> state_history_cov;
		std::deque<Eigen::Matrix<double, 16, 16>> state_history_pred_cov;
		std::deque<Eigen::Matrix<double, 16, 16>> state_history_updated_cov_;

		void lidar_points_manipulation2(const Pose&);
		void lidar_points_manipulation(double, double, double, const Pose&);
		double delta_x;
		double delta_y;
		double delta_time;
		double velocity_deltax;
		double velocity_deltay;
		int is_gnss_initialized;
		Pose state_to_pose(const State&);
};
